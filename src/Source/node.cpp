...
Node::Node(Mesh* mesh, Shader* shader, const glm::mat4& transform)
    : parent(nullptr), leftChild(nullptr), rightChild(nullptr), mesh(mesh), shader(shader), transform(transform), id(nextNodeId++) {
}
...
void Node::Draw(const Camera& camera) {
    ...
    if (leftChild) leftChild->Draw(camera);
    if (rightChild) rightChild->Draw(camera);
}